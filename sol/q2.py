class FileValidationError(Exception):
    def __init__(self, file_path, missing_lines=None, missing_keyword=None, message="File validation failed"):
        ...
        # Enhance the initial error message by appending specific details about the validation failure.
...
def validate_file_contents(file_path, expected_lines, contains_keyword):
    try:
        with open(file_path, 'r') as file:
            # Read lines from the file and determine the actual number of lines.
            ...
            if actual_lines != expected_lines:
                ...
                raise FileValidationError(file_path, missing_lines=missing_lines)
            
            # Check if the file contents contain a specific keyword.
            # If the keyword is not found, raise FileValidationError with the missing keyword.
            file_content = ''.join(lines)
            if contains_keyword not in file_content:
                raise FileValidationError(file_path, missing_keyword=contains_keyword)
